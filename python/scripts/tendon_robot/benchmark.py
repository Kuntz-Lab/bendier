...
class TendonRobotBaseline:
    def __init__(self, K_inv, num_discs, rod_length, holes):
        self.K_se_inv = np.array(K_inv[3:,3:])
        self.K_bt_inv = np.array(K_inv[:3,:3])
        self.holes = np.array(holes)

        self.num_discs = num_discs
        self.s_discs = np.linspace(0, rod_length, num_discs)

        # Solving for the external force at each disc, including tip force
        self.x0 = np.zeros(6 * num_discs)
